...
![龙虾图书馆预览截图](./public/皮皮虾大别墅_preview.jpg)
...
龙虾图书馆 / 皮皮虾大别墅 会把 OpenClaw 相关资源映射到不同房间，例如：

![龙虾图书馆预览动图](./public/皮皮虾大别墅_preview-6s.gif)
...
```text
请帮我安装并启动这个仓库：克隆 https://github.com/shengyu-meng/皮皮虾大别墅 到本地，进入项目目录执行 npm install、npm run validate，然后启动开发服务，并把最终访问地址告诉我。
```
...
龙虾图书馆 / 皮皮虾大别墅 现在使用根目录的 `clawlibrary.config.json` 作为公开版配置入口。
...
- 如果你把 皮皮虾大别墅 绑定到 `0.0.0.0` 或其他非本机地址，皮皮虾大别墅 会优先使用 `auth.password` / `CLAWLIBRARY_ACCESS_PASSWORD`，否则会在启动时提示你为当前进程输入一次访问密码。
- 如果是非交互式启动，请提前配置 `auth.password` 或 `CLAWLIBRARY_ACCESS_PASSWORD`，因为这时不会有终端密码提示。

## OpenClaw 路径探测

龙虾图书馆 / 皮皮虾大别墅 不会写死到某一台电脑的绝对路径。
