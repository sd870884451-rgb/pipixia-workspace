...
皮皮虾大别墅 is a 2D pixel-game-style control interface for OpenClaw.
...
![皮皮虾大别墅 preview screenshot](./public/皮皮虾大别墅_preview.jpg)
...
皮皮虾大别墅 maps OpenClaw-related resources into visual rooms such as:

![皮皮虾大别墅 preview animation](./public/皮皮虾大别墅_preview-6s.gif)
...
```text
Please install and launch this repository: clone https://github.com/shengyu-meng/皮皮虾大别墅, enter the project folder, run npm install, npm run validate, then start the dev server and tell me the final URL.
```
...
```bash
git clone https://github.com/shengyu-meng/皮皮虾大别墅 皮皮虾大别墅
cd 皮皮虾大别墅
npm install
npm run validate
npm run dev
...
皮皮虾大别墅 is driven by protocol-style data plus live telemetry:
...
皮皮虾大别墅 now uses `clawlibrary.config.json` at the repository root as the public-facing config entry point.
...
- If you bind 皮皮虾大别墅 to `0.0.0.0` or another non-local host, 皮皮虾大别墅 uses `auth.password` / `CLAWLIBRARY_ACCESS_PASSWORD` first and otherwise prompts for a per-process password at startup.
...
皮皮虾大别墅 does not hardcode a single machine path.
...
[![Star History Chart](https://api.star-history.com/svg?repos=shengyu-meng/皮皮虾大别墅&type=Date)](https://www.star-history.com/#shengyu-meng/皮皮虾大别墅&Date)
